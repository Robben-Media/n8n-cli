...
import (
	...
	"github.com/builtbyrobben/n8n-cli/internal/config"
)
...
const (
	apiKeyKey          = "api_key"
	apiURLKey          = "api_url"
	keyringPasswordEnv = "N8N_CLI_KEYRING_PASS" //nolint:gosec // env var name, not a credential
	keyringBackendEnv  = "N8N_CLI_KEYRING_BACKEND"
...
func openKeyringWithTimeout(cfg keyring.Config, timeout time.Duration) (keyring.Keyring, error) {
	...
	case <-time.After(timeout):
		return nil, fmt.Errorf("%w after %v (D-Bus SecretService may be unresponsive); "+
			"set N8N_CLI_KEYRING_BACKEND=file and N8N_CLI_KEYRING_PASS=<password> to use encrypted file storage instead",
...
func (s *KeyringStore) SetAPIURL(url string) error {
...
func (s *KeyringStore) DeleteAll() error {
	for _, key := range []string{apiKeyKey, apiURLKey} {
